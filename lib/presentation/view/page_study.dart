...
class PageStudy extends HookConsumerWidget {
  final List<ModelReview> list;
  const PageStudy({super.key, required this.list});
            ...
            child: CardSwiper(
              ...
              onSwipe: (previousIndex, currentIndex, direction) async {
                try {
                  ...
                  await ref.read(mvStudyProvider).swipe(id: card.id, direction: direction.name, reps: card.reps, inter: card.interval);
                  return true;
                } catch (e) {
                  if (context.mounted) {
                    await showDialog(
          ...
          FilledButton.icon(
            onPressed: () async {
              try {
                ...
                await ref.read(mvStudyProvider).notknow(id: card.id, reviewCount: card.reviewCount);
                index.value += 1;
